...
<div class="contentDiv">
    <div style="font-size:20px;left:0;">
        결제 정보
        <div>
            <form class="buttonForm" method="post" action="/reqBrowseAccommodationPayment">
    ...
    <div>
        ...
        <div class="modify_delete_div">
            <a id="register" class="aLink" href="#" onclick="showPR()">등록</a>
            <a id="modify" class="aLink" href="#" onclick="showPM()">수정</a>
            <a id="delete" class="aLink" href="#" onclick="showPD()">삭제</a>
        </div>
    </div>
</div>
<script>
    var radio =document.getElementsByName("selected");
    var radioSelectedRow;
    let deleteU = "${pageContext.request.contextPath}/view/accommodationPayment/";//라디오 버튼이 출력된 행 번호 저장
    ...
    function showPR(){
        window.open("/view/accommodationPayment/accommodationPaymentRegister.jsp","register","width=500, height=400, left=200, top=100, resizable = no");
    }
    function showPM() {
        checkTableRow();
        document.getElementById("modify").setAttribute("href", "/view/accommodationPayment/accommodationPaymentModify.jsp?paymentId=" + encodeURI(document.getElementsByName("paymentId")[radioSelectedRow].innerHTML)+
    ...
    function showPD() {
        ...
        for (var i = 0; i < check.length; i++) {
            if (check[i].checked) {
                deleteU = "${pageContext.request.contextPath}/view/accommodationPayment/accommodationPaymentDelete.jsp?userId=" + encodeURI(check[i].value);
            }
        }
        window.open(deleteU, "delete", "width=500, height=500, left=100, top=50, resizable=no");
    }
</script>
</body>
</html>
